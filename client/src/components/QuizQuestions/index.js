...
class QuizQuestions extends Component {
    ...
    renderChoices = () => {
        return this.state.shuffledQuestions[this.state.index].choices.map((choice) => (
            <ToggleButton type="radio" value={choice} key={choice} size="lg" block className="my-3 choicesBtn">
                {choice}
            </ToggleButton>
        ));
    }
    ...
    renderNextButton = () => {
        const nextQuestion = (
            <Button size="lg" className="my-3 nextBtn" disabled={!this.state.isChosen} onClick={this.nextQuestion}>
