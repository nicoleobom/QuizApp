...
class Quiz extends Component {
    ...
    updateUserScore = async () => {
        ...
        if (user.scores.length > 0) {
            currentHighscore = user.scores[0].score;
        } else {
        ...
        if (!currentHighscore || currentHighscore == null || this.state.score > currentHighscore){
            API.updateUserScore(user._id, userData);
        }
    }
    ...
    renderTimer = () => {
        ...
        const color = this.state.timer <= 10 ? "red" : "white";
